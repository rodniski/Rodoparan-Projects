...
const Timeline: React.FC<TimelineProps> = ({ events, isOpen, onClose }) => {
    ...
    switch (status.toLowerCase()) {
      ...
      case "a classificar":
        return (
          <Badge
            variant="outline"
            className="text-blue-500 bg-transparent ring-1 ring-blue-600 hover:text-blue-50 hover:bg-blue-700"
          >
            A Classificar
          </Badge>
        );
      case "revisar":
        return (
          <Badge
            variant="outline"
            className="text-red-500 bg-transparent ring-1 ring-red-600 hover:text-red-50 hover:bg-red-700"
          >
            Revisar
          </Badge>
        );
    ...
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent>
        <SheetHeader>
          ...
          <SheetDescription>Detalhes das ações realizadas.</SheetDescription>
        </SheetHeader>
          ...
          <div className="relative pl-6 grid gap-10 after:absolute after:inset-y-0 after:left-0 after:w-px after:bg-muted-foreground/20">
            {events.map((event, index) => (
              <div key={index} className="grid gap-1 text-sm relative">
                <div
                  className={`aspect-square w-3 rounded-full absolute left-0 translate-x-[-29.5px] z-10 top-1 ${getStatusColor(
                    event.status
                  )}`}
                />
